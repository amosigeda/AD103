...
public class SqlMapDirectiveInfoDao extends SqlMapClientDaoSupport implements DirectiveInfoDao{
	...
	public List<DataMap> getImsiAndResourceInfo(DirectiveInfo vo) {
		logger.debug("getImsiAndResourceInfo(DirectiveInfo vo)");
		return getSqlMapClientTemplate().queryForList("getImsiAndResourceInfo", vo);
	}

	public Integer insertLog(DirectiveInfo vo) throws DataAccessException {
		logger.debug("insertLog(DirectiveInfo vo)");
		return getSqlMapClientTemplate().update("insertLog", vo);
	}
}
